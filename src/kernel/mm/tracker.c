...
static inline uintptr_t uadd_overflow(uintptr_t a, uintptr_t b, uintptr_t *out) {
  uintptr_t c = a + b;
  if (c < a) return 1;
  *out = c; return 0;
}
...
static inline int height(tracker_node_t *n) {
  return n ? n->height : 0;
}
...
static void pull_up(tracker_node_t *n) {
  ...
  n->max_free = max_sz(n->len, 
                       max_sz(subtree_max_free(n->left), 
                              subtree_max_free(n->right)));
}
...
static tracker_node_t* avl_insert_exact(tracker_node_t *root, uintptr_t start, size_t len, bool *ok) {
    if (!root) {
        *ok = true;
    ...
    if (bf > 1 && start < root->left->start) return rotate_right(root);
    if (bf < -1 && start > root->right->start) return rotate_left(root);
    if (bf > 1 && start > root->left->start)  { root->left = rotate_left(root->left); return rotate_right(root); }
    if (bf < -1 && start < root->right->start){ root->right = rotate_right(root->right); return rotate_left(root); }

    return root;
}

static tracker_node_t* min_node(tracker_node_t *n) { while (n && n->left) n = n->left; return n; }

static tracker_node_t* avl_delete_by_start(tracker_node_t *root, uintptr_t start, tracker_node_t **removed) {
    if (!root) return NULL;
    ...
    if (bf > 1 && balance_factor(root->left) <  0) { root->left = rotate_left(root->left); return rotate_right(root); }
    if (bf < -1 && balance_factor(root->right) <= 0) return rotate_left(root);
    if (bf < -1 && balance_factor(root->right) >  0) { root->right = rotate_right(root->right); return rotate_left(root); }
...
static tracker_node_t* find_fit(tracker_node_t *n, size_t size, size_t align, uintptr_t *alloc_start_out) {
    if (!n || subtree_max_free(n) < size) return NULL;

    // Try left first
    tracker_node_t *left_fit = find_fit(n->left, size, align, alloc_start_out);
    if (left_fit) return left_fit;
...
bool tracker_add_free(tracker_tree_t *t, uintptr_t start, size_t len) {
    ...
    uintptr_t end;
    if (uadd_overflow(start, len, &end)) return false;
...
// Insert with coalescing (the one you should call)
    ...
    if (R) {
        if (end == R->start) {
            ...
            if (removed) {
                ...
                uintptr_t new_end;
                if (uadd_overflow(end, removed->len, &new_end)) { kfree(removed); return false; }
                end = new_end;
                len = (size_t)(end - start);
                kfree(removed);
            }
...
// Allocate `size` bytes with `align` (0 or power-of-two is best). Returns true/false.
// On success, *out_addr is set.
bool tracker_alloc(tracker_tree_t *t, size_t size, size_t align, uintptr_t *out_addr) {
    ...
    uintptr_t alloc_end;
    if (uadd_overflow(alloc_start, size, &alloc_end)) return false;
